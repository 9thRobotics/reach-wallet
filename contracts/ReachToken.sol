...
contract ReachToken is IERC20 {
    ...
    event VoteCast(address indexed voter, uint256 indexed proposalId, uint256 votes); // votes will always be 1 for "1 vote per wallet"
    ...
    // GOVERNANCE FUNCTIONS - "1 vote per wallet" system
    ...
    /**
     * @dev Vote on a proposal - 1 vote per wallet regardless of token balance
     * @param proposalId The ID of the proposal to vote on
     */
    function vote(uint256 proposalId) external whenNotPaused {
        require(proposalId > 0 && proposalId <= proposalCount, "Invalid proposal ID");
        require(balanceOf(msg.sender) > 0, "Must hold tokens to vote"); // Basic eligibility check
